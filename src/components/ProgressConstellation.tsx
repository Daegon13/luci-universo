...
export function ProgressConstellation({ visitedCount, totalCount, isSecretUnlocked, performanceMode = "balanced" }: ProgressConstellationProps) {
  const completion = Math.min(100, Math.round((visitedCount / totalCount) * 100));

  return (
    <div className={`relative mt-5 overflow-hidden rounded-2xl border border-violet-200/20 bg-[#120d28]/80 p-4 lg:mt-6 lg:flex lg:items-center lg:gap-6 lg:p-5 ${performanceMode === "lite" ? "shadow-none" : ""}`}>
      ...
      {isSecretUnlocked && performanceMode !== "lite" ? <StarBurst active className="left-auto right-8 top-5 h-10 w-10" /> : null}
      ...
      <div className="relative mt-3 flex-1 lg:mt-0">
        <div className="h-2 w-full overflow-hidden rounded-full bg-violet-200/15">
          <m.div
            ...
            transition={{ duration: performanceMode === "lite" ? 0.22 : 0.55, ease: "easeOut" }}
          />
        </div>
